...
bool TransparentShader::initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = initializeShader(device, hwnd, L"./hlsl/TransparentVertexShader.hlsl", L"./hlsl/TransparentPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TransparentShader::render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool TransparentShader::initializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
	...
	fogBufferDesc.ByteWidth = sizeof(TransparentBufferType);
	fogBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	fogBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	fogBufferDesc.MiscFlags = 0;
	fogBufferDesc.StructureByteStride = 0;

	result = device->CreateBuffer(&fogBufferDesc, NULL, &m_transparentBuffer);
	if (FAILED(result))
	{
		return false;
	}
...
void TransparentShader::outputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool TransparentShader::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
									D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float blend)
{
	HRESULT result;
