...
bool ReflectionShader::initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = initializeShader(device, hwnd, L"./hlsl/ReflectionVertexShader.hlsl", L"./hlsl/ReflectionPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool ReflectionShader::render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
							  D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTexture, 
...
bool ReflectionShader::initializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	lightBufferDesc.ByteWidth = sizeof(ReflectionBufferType);
	lightBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER; 
	lightBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	lightBufferDesc.MiscFlags = 0; 
	lightBufferDesc.StructureByteStride = 0;

	result = device->CreateBuffer(&lightBufferDesc, NULL, &m_reflectionBuffer);
	if (FAILED(result)) {
		return false;
	}
