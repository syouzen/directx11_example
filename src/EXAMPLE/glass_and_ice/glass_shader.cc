...
bool GlassShader::initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = initializeShader(device, hwnd, L"./hlsl/GlassVertexShader.hlsl", L"./hlsl/GlassPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool GlassShader::render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
						 ...
						 float refractionScale)
{
	...
	result = setShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, colorTexture, 
								 refractionTexture, normalTexture, refractionScale);
	if (!result)
	{
		return false;
	}
...
bool GlassShader::initializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
void GlassShader::outputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool GlassShader::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
									  D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* colorTexture,
									  ID3D11ShaderResourceView* normalTexture, ID3D11ShaderResourceView* refractionTexture,
									  float refractionScale)
{
