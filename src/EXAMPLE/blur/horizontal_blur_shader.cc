...
bool HorizontalBlurShader::initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	
	result = initializeShader(device, hwnd, L"./hlsl/HorizontalBlurVertexShader.hlsl", L"./hlsl/HorizontalBlurPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool HorizontalBlurShader::render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool HorizontalBlurShader::initializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
bool HorizontalBlurShader::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
											   D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	HRESULT result;
