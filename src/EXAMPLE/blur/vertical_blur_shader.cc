...
bool VerticalBlurShader::initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = initializeShader(device, hwnd, L"./hlsl/VerticalBlurVertexShader.hlsl", L"./hlsl/VerticalBlurPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool VerticalBlurShader::render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool VerticalBlurShader::initializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
void VerticalBlurShader::outputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool VerticalBlurShader::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
											   D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	HRESULT result;
