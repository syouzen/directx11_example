...
bool Model::initialize(ID3D11Device* device, const char* modelFilename, const WCHAR* textureFilename, const WCHAR* textureFilename2, const WCHAR* textureFilename3)
{
	bool result;
	...
	result = loadTexture(device, textureFilename, textureFilename2, textureFilename3);
	if (!result)
	{
		return false;
	}
...
bool Model::loadTexture(ID3D11Device* device, const WCHAR* filename, const WCHAR* filename2, const WCHAR* filename3)
{
	bool result;
	...
	result = m_TextureArray->initialize(device, filename, filename2, filename3);
	if (!result)
	{
		return false;
	}
